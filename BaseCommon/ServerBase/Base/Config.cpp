...
__ENTER_FUNCTION
	...
	if (m_JingJieConfig.m_jingMaiCnt > 0)
	{
		...
		AssertEx(m_JingJieConfig.m_bonusAttrLv1Point >= 0, "打通经脉时奖励的1级属性值非法!");
		m_JingJieConfig.m_bonusPointTimer = ini.ReadInt("JingJieSystem", "BonusPointTimer");
		AssertEx(m_JingJieConfig.m_bonusPointTimer >= 1000, "在线定时奖励的修炼点值的时间间隔太短,将会严重影响服务器性能与速度!");
		m_JingJieConfig.m_trainingPointMultiple = ini.ReadFloat("JingJieSystem", "TrainingPointMultiple");
		AssertEx(m_JingJieConfig.m_trainingPointMultiple >= 1.0f && m_JingJieConfig.m_trainingPointMultiple <= 8.0f, "修炼点倍数非法!");
		m_JingJieConfig.m_expressTrainingpointSpeed = ini.ReadInt("JingJieSystem", "ExpressTrainingpointSpeed");
		AssertEx(m_JingJieConfig.m_expressTrainingpointSpeed >= 1, "修炼点的传功速度值非法!");
		m_JingJieConfig.m_onlineBonusPoint = ini.ReadInt("JingJieSystem", "OnlineBonusPoint");
		AssertEx(m_JingJieConfig.m_onlineBonusPoint >= 0, "在线定时奖励的修炼点值非法!");
		...
		for (INT iIndex = 0; iIndex < MAX_JINGJIE_LEVEL; iIndex++)
		{
			...
			AssertEx(m_JingJieConfig.m_limitTrainingPoint[iIndex] >= 0 &&
				m_JingJieConfig.m_limitTrainingPoint[iIndex] <= INT_MAX, "相应境界的修炼点存储上限值非法!");

			AssertEx(m_JingJieConfig.m_expressTrainingpointSpeed <= m_JingJieConfig.m_limitTrainingPoint[iIndex], "在当前修炼点值存储上限的前提下修炼点的传功速度值是非法的!");
		}
	}
...
__ENTER_FUNCTION
	...
	{
		AssertEx(FALSE, "第一个开怪的个人或队伍归属权的秒数 配置非法!");
	}
	...
	m_ConfigInfo.m_DefaultCheckMisSumbitTagRate = ini.ReadInt("TimeSetting", "DefaultCheckMisSumbitTagRate"); // 检测可提交任务的标识的间隔时间 add by zengwen
	AssertEx(m_ConfigInfo.m_DefaultCheckMisSumbitTagRate >= 1000, "检测可提交任务的标识的间隔时间设置的太短,这样会严重引响服务端性能与速度!");

	m_ConfigInfo.m_RedValue = ini.ReadInt("PK","redValue");;					// 		redValue=100;红名
	m_ConfigInfo.m_BlackValue = ini.ReadInt("PK","blackvalue");					// 		blackvalue=300;黑名
	m_ConfigInfo.m_KillerAddValue  = ini.ReadInt("PK","killerAddValue");		// 		killerAddValue=100;凶手增加的杀气
	m_ConfigInfo.m_HelperAddValue  = ini.ReadInt("PK","helperAddValue");		// 		helperAddValue=50;帮凶增加的杀气
	m_ConfigInfo.m_PkProtectLevel  = ini.ReadInt("PK","pkProtectLevel");		// 		pkProtectLevel=10;PK保护等级
...
VOID	Config::LoadShareMemInfo_Only()
{
	__ENTER_FUNCTION
		...
		// 读取消息传递用到的共享内存的KEY [杨文鸽 2011/2/20]
		INT nServerCount=(INT)ini.ReadInt( "MsgKey", "ServerCount" );
		AssertEx(nServerCount>0,"配置错误:通过共享内存传递消息用于的KEY的服务器数量必须大于0(尝试SVN更新一下共享内存配置文件)");
...
__ENTER_FUNCTION
	...
	for( SceneID_t i=0; i<m_SceneInfo.m_SceneCount; i++ )
	{
		...
		m_SceneInfo.m_pScene[i].m_ResID = ini.ReadInt( szSection, "clientres" ) ;//客户端使用资源ID add by zcx 2010.2.8
...
{//通过资源ID获取服务器ID add by zcx 2010.2.8
	__ENTER_FUNCTION
...
{//通过资源ID获取场景ID add by zcx 2010.2.8
	__ENTER_FUNCTION
...
{//通过场景获取资源ID add by zcx 2010.2.8
	__ENTER_FUNCTION
