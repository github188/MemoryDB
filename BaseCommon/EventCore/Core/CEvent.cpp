...
namespace Logic
{
	...
	{
		if (mEventFactory)
			return mEventFactory->mEventCenter;
		else
			return NULL;
	}
...
#define  _USE_NICE_PROTOCOL_  0
	bool BaseEvent::_Serialize( DataStream *destData )
	{
		destData->write(mTarget);
	...
	bool BaseEvent::_Restore(DataStream *scrData )
	{
        ...
        if (saveState==eEventSaveDefault)
        {
            ...
            TABLE_LOG("*<%s> Event all nice data size %d", GetEventName(), nowSize-scrData->dataSize());
            return bRe;
#else
...
#if DEVELOP_MODE
                TABLE_LOG("<%s> Protocal data size = %d, nice data protocal data size %d", GetEventName(), temp.dataSize(), nowSize - scrData->dataSize());
                extern char gLogPathFile[256];
                AString logFile = Allot::gLogPathFile;
                logFile += "_protocol.txt";
                GetEventCenter()->mProtocolMgr.SaveAllProtocol(logFile.c_str(), false);
        ...
        {
            TABLE_LOG("<%s> Event nice data size %d", GetEventName(), nowSize-scrData->dataSize());
	...
	bool BaseEvent::set( const char* szKey, AutoRecord &hRecord )
	{
	...
	bool BaseEvent::get( const char* szKey, AutoRecord &hRecord )
	{
	...
	bool BaseEvent::WaitTime( float waitTime, EventCallBack callBack )
	{
		return GetEventCenter()->_StartWaitTime(GetSelf(), waitTime, callBack);
	}
