...
public:
	...
	~TotalDebug()
	{
		...
		for (EasyList<Logic::tLogicNode*>::iterator it=mLogicList.begin(); it!=mLogicList.end(); ++it)
		{
			//(*it)->GetData().dump();
		}
	}
...
namespace Logic
{
	...
	{
		EasyList<EasyString> &eventList = GetLogicFactory()->getNextActionList();
		for (EasyList<EasyString>::iterator it=eventList.begin(); it!=eventList.end(); ++it)
		{
			EasyString &str = (*it);
	...
	{
		EasyList<EasyString> &eventList = GetLogicFactory()->getEventList();
		for (EasyList<EasyString>::iterator it=eventList.begin(); it!=eventList.end(); ++it)
		{
