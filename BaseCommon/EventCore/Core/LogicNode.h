...
namespace Logic
{
	...
	{
	public:
		void addEvent(const char* actionEventName){ mEventList.insert(EasyString(actionEventName)); }
		void addNextLogic(const char* nextLogic){ mNextEvent.insert(EasyString(nextLogic)); }

		void removeEvent(const char* actionEventName){ mEventList.remove(EasyString(actionEventName)); }
		void removeNextLogic(const char* nextLogic){ mNextEvent.remove(EasyString(nextLogic)); }

		EasyList<EasyString>& getEventList(void){ return mEventList; }
		EasyList<EasyString>& getNextActionList(void){ return mNextEvent; }

	protected:
		EasyList<EasyString>	mNextEvent;
		EasyList<EasyString>	mEventList;
	};
